#!/usr/bin/env python3
"""Fix Deck 2 artist - try different x_start and x_end combinations."""
import numpy as np
...
print("Testing ARTIST region with different x_start and x_end:")
print("="*60)
...
test_cases = [
    (0.05, 0.12, "Skip beginning"),
    (0.06, 0.13, "Narrower"),
    (0.07, 0.14, "Even narrower"),
    (0.08, 0.15, "Focus on Outkast"),
    (0.09, 0.16, "Wider end"),
    (0.10, 0.17, "Wider"),
]

for x_start_pct, x_end_pct, desc in test_cases:
    artist_x_start = int(deck_width * x_start_pct)
    artist_x_end = int(deck_width * x_end_pct)
    ...
    is_good = (artist_text and 
               ...
               "mey Ta" not in artist_text and
               "ney Ta" not in artist_text)
    
    status = "✅ PERFECT" if is_perfect else ("✅" if is_good else "❌")
    print(f"{status} {desc}: x_start={x_start_pct:.2f}, x_end={x_end_pct:.2f} → '{artist_text}'")
    
    if is_perfect:
        print(f"  → PERFECT! Use x_start={x_start_pct:.4f}, x_end={x_end_pct:.4f}")
        artist_image.save(f"deck2_artist_fixed.png")
        break
